/*
  ...
  Version: v0.7.2-gospel-opportunities-boundary-layer

  Changes in this version:
  - Adds Gospel Opportunities as a fourth option in the Boundary layer dropdown.
  - Uses lea-boundaries.geojson for Gospel Opportunities.
  - Loads gospel-opportunities.csv.
  - Colours LEAs by OpportunityLevel.
  - Keeps County, LEA, and Urban Zone boundary switching.
  - Keeps church dots above polygon layers.
  - Keeps church detail panel and denomination filters.
*/
...
const boundaryConfigs = {
  ...
  gospel: {
    ...
    geojsonFile: "lea-boundaries.geojson",
    dataFile: "gospel-opportunities.csv"
  }
};
...
function getGospelOpportunityColour(level) {
  const cleanLevel = clean(level);
  return gospelOpportunityColours[cleanLevel] || gospelOpportunityColours["No Data"];
}
...
function showOrHideGospelLegend() {
  ...
  legend.style.display =
    currentBoundaryType === "gospel" ? "block" : "none";
}
...
function updateProfilePanel(boundaryName, boundaryLeafletLayer) {
  ...
  if (currentBoundaryType === "gospel") {
    ...
    summary.innerHTML = `
      ...
      <strong>Opportunity level:</strong> ${escapeHtml(opportunityLevel)}<br>
  ...
  fetch(withCacheBust(config.geojsonFile))
    ...
    .then((geojson) => {
      ...
      currentBoundaryLayer = L.geoJSON(geojson, {
        pane: boundaryType === "gospel" ? "gospelOpportunityPane" : "boundaryPane",
        style: boundaryStyle,
